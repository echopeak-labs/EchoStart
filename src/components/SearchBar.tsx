...
export const SearchBar = () => {
  ...
  return (
    <form onSubmit={handleSearch} className="flex items-center gap-2 w-full max-w-2xl">
      ...
      <div className="flex-1 flex glass rounded-xl overflow-hidden shadow-glass">
        <div className="relative">
          <button
            type="button"
            onClick={() => setShowEngines(!showEngines)}
            className="h-full px-4 flex items-center gap-2 glass-hover transition-smooth text-sm font-medium"
          >
            {selectedEngine.name}
          ...
          {showEngines && (
            <div className="absolute top-full left-0 mt-2 glass-strong rounded-lg overflow-hidden shadow-glass z-50 animate-scale-in">
              {searchEngines.map((engine) => (
                <button
                  ...
                  className="w-full px-4 py-2 text-left text-sm hover:bg-primary/20 transition-smooth whitespace-nowrap"
                >
                  {engine.name}
                </button>
              ))}
